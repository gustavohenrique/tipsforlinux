...
from socialauth.forms import EditProfileForm, SigninUserForm
...
def _show_login_page(request, auth_form=None, signin_form=None):
    if request.user.is_authenticated():
        return HttpResponseRedirect(reverse('socialauth_editprofile'))
...
def signin_common_confirm(request, user_id, hash):
    try:
        ...
        if hash == sha.new(user.email).hexdigest():
            user.is_active = True
            user.save()
            return HttpResponseRedirect(reverse('socialauth_success_signin_common_confirmation'))
    except:
        pass
...
def _send_mail_confirmation(request, user):
    ...
    hash = sha.new(user.email).hexdigest()
  
    link = 'http://%s%s' % (request.META['SERVER_NAME'], reverse('socialauth_confirm_signin_common', args=[user.id, hash]))
    subject = 'Confirm your TipsForLinux account'
    message = """
    Hi!
    
    Someone just entered this email address on TipsForLinux.
    ...
    if request.POST:
        ...
        if signin_form.is_valid():            
            try:
                ...
                user.email = signin_form.cleaned_data['email']
                user.save()
                _send_mail_confirmation(request, user)
                return HttpResponseRedirect(reverse('socialauth_success_signin_common'))
            except:
                return HttpResponse('Error trying to send e-mail confirmation. Please contact the Administrator.')
        ...
        if user is not None:
            if user.is_active:
                ...
                if 'openid_next' in request.session:
                    return HttpResponseRedirect(request.session.get('openid_next'))
                
                return HttpResponseRedirect(reverse('tip-latest'))
    
        auth_form = AuthenticationForm()
        auth_form.errors.update({u'': u'username or password incorrect'})
        return _show_login_page(request, auth_form)
...
def openid_done(request, provider=None):
    ...
    if  request.openid:
        ...
        if user:
            ...
            redirect_url = reverse('tip-latest')
